...
public class FileUploadByHttpClient {
    public static void main(String[] args) throws Exception {
        ...
        for (File file : files) {
            ...
            try {
                HttpPost httppost = new HttpPost("http://10.185.101.4/HamsPort/uploadEfile");
                FileBody bin = new FileBody(ff);

                HttpEntity reqEntity = MultipartEntityBuilder.create()
                        .addPart("Filedata", bin)
                        //可能出现中文这样调用 需要 utf ,注意title不要后缀
                        .addTextBody("title", FilenameUtils.getBaseName(file.getName()), utf8)
                        //写成可以配置的正式环境和测试环境不同 测试是这个值
                        .addTextBody("tableName","E_FILE29")
                        //上次调用返回的did
                        .addTextBody("fileDID","4")
                        //写成可以配置的正式环境和测试环境不同
